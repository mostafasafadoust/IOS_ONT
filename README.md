...
5. GitHub builds the app on a hosted macOS runner.
6. Download `TivanONTiOS-unsigned-ipa` for signing with sideload tools such as 3uTools.
...
The workflow produces `TivanONTiOS-unsigned-ipa`, an unsigned `.ipa` package intended for external signing tools. This packaging follows the same structure as the working TorchTest project: build `Release-iphoneos`, copy `TivanONTiOS.app` into `Payload`, then zip `Payload` as `TivanONTiOS-unsigned.ipa`.

Download the GitHub artifact, extract the outer ZIP once, then select `TivanONTiOS-unsigned.ipa` in 3uTools. Do not select the extracted `Info.plist`, `PkgInfo`, `Payload` folder, or the raw `TivanONTiOS` executable.
