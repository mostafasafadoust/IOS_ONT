...
6. Download `TivanONTiOS-simulator-app` for Simulator tests, or `TivanONTiOS-adhoc-iphoneos` for signing with sideload tools such as 3uTools.
...
The workflow also produces `TivanONTiOS-adhoc-iphoneos`, an ad-hoc signed `.ipa` package intended for external signing tools. If the signing tool asks for an IPA, select `TivanONTiOS-adhoc-iphoneos.ipa`; do not select the extracted `Info.plist`, `PkgInfo`, or the raw `TivanONTiOS` executable.
