...
6. Download `TivanONTiOS-simulator-app` for Simulator tests, or `TivanONTiOS-unsigned-iphoneos` for signing with sideload tools such as 3uTools.
...
The workflow also produces `TivanONTiOS-unsigned-iphoneos`, an unsigned `.ipa` package intended for external signing tools. If the signing tool asks for an IPA, select `TivanONTiOS-unsigned-iphoneos.ipa`; do not select the extracted `Info.plist`, `PkgInfo`, or the raw `TivanONTiOS` executable.
