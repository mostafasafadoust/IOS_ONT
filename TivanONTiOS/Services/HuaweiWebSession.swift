...
final class HuaweiWebSession: NSObject, ObservableObject {
    ...
    func login(_ configuration: BootstrapConfiguration) async throws -> ScriptResult {
        ...
        for _ in 0..<8 {
            ...
            after = try await evaluateScriptResult(JavaScriptPayloads.loginState)
            if after.logged == true {
                return after
            }
        }

        if let wanURL = configuration.url(path: "/html/bbsp/wan/wan.asp", scheme: activeScheme) {
            try? await load(wanURL, timeout: 8)
            if let direct = try? await evaluateScriptResult(JavaScriptPayloads.loginState),
               direct.logged == true {
                return direct
            }
        }
        ...
        throw BootstrapError.scriptFailed(
            "ورود به پنل مودم تایید نشد. submit=\(submitted.compactSummary) | after=\(after.compactSummary)"
        )
    }
    ...
    private func load(_ url: URL, timeout: TimeInterval) async throws {
        ...
        try await withCheckedThrowingContinuation { continuation in
            ...
            loadTimeoutTask = Task { [weak self] in
                ...
                self?.finishLoad(.failure(BootstrapError.timeout(url.absoluteString)))
            }
        }
    }
