...
enum JavaScriptPayloads {
    ...
    (function(){
      ...
      const text=allDocs.map(d=>((d.body&&d.body.innerText)||'')).join('\n').toLowerCase();
      const inputs=allDocs.flatMap(d=>[...d.querySelectorAll('input')]);
      const hasPass=inputs.some(x => (x.type||'').toLowerCase()==='password' && visible(x));
      const logged =
        !hasPass &&
        (
          /logout|log out|wan|tr-?069|system information|device information|status|overview|device info/.test(text) ||
          /\/html\//i.test(location.href)
        );
      const diagnostics=inputs.slice(0,30).map(x=>[
        x.type||'text',
        x.id||'',
        x.name||'',
        x.placeholder||'',
        visible(x)?'visible':'hidden'
      ].join(':'));
    ...
    static func login(username: String, password: String) -> String {
        ...
        (async function(){
          ...
          function setv(e,v){
            ...
            for(const n of ['input','change','keyup','keydown','blur']) {
              try{ e.dispatchEvent(new Event(n,{bubbles:true})); }catch(err){}
            }
            return true;
          }
          function norm(e){
            return ((e.id||'')+' '+(e.name||'')+' '+(e.placeholder||'')+' '+(e.title||'')+' '+(e.autocomplete||'')).toLowerCase();
          }
          function byExact(doc, ids, names){
            for(const id of ids){
          ...
          function fireClick(e){
            if(!e)return false;
            for(const n of ['pointerdown','mousedown','touchstart','pointerup','mouseup','touchend']) {
              try{ e.dispatchEvent(new Event(n,{bubbles:true,cancelable:true})); }catch(err){}
            }
            e.click();
            return true;
          }
          function scan(doc){
            ...
            const pass=byExact(
              doc,
              ['txt_Password','Password','password','UserPassword','userPassword','Frm_Password','login_password','loginPassword'],
              ['txt_Password','Password','password','UserPassword','userPassword','Frm_Password','login_password','loginPassword']
            ) || visibleInputs.find(x=>(x.type||'').toLowerCase()==='password') ||
              allInputs.find(x=>(x.type||'').toLowerCase()==='password');
            if(!pass) {
              return {ok:false,error:'password field not found',diagnostics:allInputs.slice(0,30).map(norm)};
            }
            const user=byExact(
              doc,
              ['txt_Username','Username','username','UserName','userName','Frm_Username','login_username','loginUser','account'],
              ['txt_Username','Username','username','UserName','userName','Frm_Username','login_username','loginUser','account']
            ) || visibleInputs.find(x=>{
              const t=(x.type||'text').toLowerCase();
              return t!=='password' && t!=='hidden' && /user|account|name|login|admin/.test(norm(x));
            }) || visibleInputs.find(x=>{
              const t=(x.type||'text').toLowerCase();
              return !['password','hidden','button','submit','checkbox','radio'].includes(t);
            }) || allInputs.find(x=>{
              const t=(x.type||'text').toLowerCase();
              return !['password','hidden','button','submit','checkbox','radio'].includes(t);
            });
            if(!user) {
              return {ok:false,error:'username field not found',passId:pass.id||pass.name||'',diagnostics:allInputs.slice(0,30).map(norm)};
            }
            setv(user,USER); setv(pass,PASS);
            const els=[
              ...doc.querySelectorAll('button,input[type=submit],input[type=button],input[type=image],a,[role=button]')
            ].filter(visible);
            let btn=els.find(e=>{
              const z=((e.innerText||'')+' '+(e.value||'')+' '+(e.id||'')+' '+(e.name||'')+' '+(e.title||'')).toLowerCase();
              return /login|log in|ورود|submit|ok|apply|btnlogin|button_login/.test(z);
            }) || doc.getElementById('loginbutton') || doc.getElementById('LoginId') || doc.getElementById('btnLogin') || els[0];
            if(btn){
              fireClick(btn);
              return {
                ok:true,
                method:'click',
                userId:user.id||user.name||'field',
                passId:pass.id||pass.name||'field',
                buttonId:btn.id||btn.name||btn.value||btn.innerText||'button',
                diagnostics:allInputs.slice(0,20).map(norm)
              };
            }
            if(pass.form){
              ...
              return {
                ...
                userId:user.id||user.name||'field',
                passId:pass.id||pass.name||'field',
                formId:pass.form.id||pass.form.name||'form',
                diagnostics:allInputs.slice(0,20).map(norm)
              };
            }
            for(const fn of ['SubmitForm','submitForm','LoginSubmit','loginSubmit','OnLogin','onLogin','CheckForm','checkForm']){
              try{
                if(typeof doc.defaultView[fn]==='function'){
                  doc.defaultView[fn]();
                  return {ok:true,method:'function',functionName:fn,userId:user.id||user.name||'',passId:pass.id||pass.name||''};
                }
              }catch(e){}
            }
            return {
              ok:false,
              error:'login button/form not found',
              userId:user.id||user.name||'',
              passId:pass.id||pass.name||'',
              diagnostics:allInputs.slice(0,30).map(norm)
            };
          }
          let r=null;
